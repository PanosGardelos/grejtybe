...
class Permissions extends BitField_1.BitField {
    ...
    any(permission, checkAdmin = true) {
        return ((checkAdmin && super.any(Permissions.FLAGS.ADMINISTRATOR)) ||
    ...
    has(permission, checkAdmin = true) {
        return ((checkAdmin && super.has(Permissions.FLAGS.ADMINISTRATOR)) ||
    ...
    hasThrow(permission) {
        if (this.has(permission) && this.has("VIEW_CHANNEL"))
            return true;
        throw new lambert_server_1.HTTPError(`You are missing the following permissions ${permission}`, 403);
    }
...
async function getPermission(user_id, guild_id, channel_id, opts = {}) {
    if (!user_id)
        throw new lambert_server_1.HTTPError("User not found");
    let channel;
    let member;
    let guild;
    if (channel_id) {
