...
(function (ChannelType) {
    ...
    ChannelType[ChannelType["UNHANDLED"] = 255] = "UNHANDLED";
...
let Channel = Channel_1 = class Channel extends BaseClass_1.BaseClass {
    ...
    static async createChannel(channel, user_id = "0", opts) {
        ...
        if (!opts?.skipNameChecks) {
            ...
            if (!guild.features.includes("ALLOW_INVALID_CHANNEL_NAMES") &&
                channel.name) {
                for (const character of util_1.InvisibleCharacters)
                    if (channel.name.includes(character))
                        throw new lambert_server_1.HTTPError("Channel name cannot include invalid characters", 403);
                ...
                if (channel.type !== ChannelType.GUILD_CATEGORY ||
                    ...
                    if (channel.name.match(/--+/g))
                        throw new lambert_server_1.HTTPError("Channel name cannot include multiple adjacent dashes.", 403);
                    if (channel.name.charAt(0) === "-" ||
                        channel.name.charAt(channel.name.length - 1) === "-")
                        throw new lambert_server_1.HTTPError("Channel name cannot start/end with dash.", 403);
                }
                else
            ...
            if (!guild.features.includes("ALLOW_UNNAMED_CHANNELS")) {
                if (!channel.name)
                    throw new lambert_server_1.HTTPError("Channel name cannot be empty.", 403);
            }
        }
        switch (channel.type) {
            ...
            case ChannelType.GUILD_VOICE:
                if (channel.parent_id && !opts?.skipExistsCheck) {
                    const exists = await Channel_1.findOneOrFail({
        ...
        channel = {
            ...
            created_at: new Date(),
            position: (channel.type === ChannelType.UNHANDLED
                ? 0
                : channel.position) || 0,
        };
        await Promise.all([
            Channel_1.create(channel).save(),
            !opts?.skipEventEmit
                ? (0, util_1.emitEvent)({
    ...
    static async createDMChannel(recipients, creator_user_id, name) {
        ...
        **/
        ...
        if (channel == null) {
            ...
            channel = await Channel_1.create({
                name,
                type,
                owner_id: undefined,
                created_at: new Date(),
                last_message_id: undefined,
                recipients: channelRecipients.map((x) => Recipient_1.Recipient.create({
                    user_id: x,
                    closed: !(type === ChannelType.GROUP_DM ||
                        x === creator_user_id),
                })),
                nsfw: false,
            }).save();
    ...
    static async removeRecipientFromChannel(channel, user_id) {
        ...
        await (0, util_1.emitEvent)({
            ...
            data: {
                channel_id: channel.id,
                user: await User_1.User.findOneOrFail({
                    where: { id: user_id },
