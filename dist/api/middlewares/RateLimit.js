...
function rateLimit(opts) {
    return async (req, res, next) => {
        ...
        const bucket_id = opts.bucket ||
            req.originalUrl.replace(Authentication_1.API_PREFIX_TRAILING_SLASH, "");
        let executor_id = (0, api_1.getIpAdress)(req);
...
async function initRateLimits(app) {
    const { routes, global, ip, error, enabled } = util_1.Config.get().limits.rate;
    if (!enabled)
        return;
