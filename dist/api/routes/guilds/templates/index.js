...
router.get("/:code", (0, api_1.route)({}), async (req, res) => {
    ...
    const template = await util_1.Template.findOneOrFail({ where: { code: code } });
    res.json(template);
});
router.post("/:code", (0, api_1.route)({ body: "GuildTemplateCreateSchema" }), async (req, res) => {
    const { enabled, allowTemplateCreation,
    // allowDiscordTemplates,
    // allowRaws,
     } = util_1.Config.get().templates;
    if (!enabled)
        return res
            .json({
            code: 403,
    ...
    const template = await util_1.Template.findOneOrFail({
        where: { code: code },
    });
    ...
    const [guild] = await Promise.all([
        ...
        util_1.Role.create({
            ...
            mentionable: true,
            name: "@everyone",
            permissions: BigInt("2251804225").toString(),
            position: 0,
        }).save(),
    ]);
