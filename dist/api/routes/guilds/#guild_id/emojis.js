...
router.get("/", (0, api_1.route)({}), async (req, res) => {
    ...
    const emojis = await util_1.Emoji.find({
...
router.get("/:emoji_id", (0, api_1.route)({}), async (req, res) => {
    ...
    await util_1.Member.IsInGuildOrFail(req.user_id, guild_id);
    const emoji = await util_1.Emoji.findOneOrFail({
...
router.post("/", (0, api_1.route)({
    ...
    const emoji_count = await util_1.Emoji.count({
        where: { guild_id: guild_id },
    });
    ...
    const emoji = await util_1.Emoji.create({
        id: id,
        guild_id: guild_id,
        ...body,
        require_colons: body.require_colons ?? undefined,
        user: user,
        managed: false,
        animated: false,
        available: true,
    ...
    await (0, util_1.emitEvent)({
        ...
        data: {
            guild_id: guild_id,
            emojis: await util_1.Emoji.find({ where: { guild_id: guild_id } }),
        },
    });
    return res.status(201).json(emoji);
});
router.patch("/:emoji_id", (0, api_1.route)({
