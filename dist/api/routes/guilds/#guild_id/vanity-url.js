...
router.get("/", (0, api_1.route)({ permission: "MANAGE_GUILD" }), async (req, res) => {
    ...
    if (!guild.features.includes("ALIASABLE_NAMES")) {
        const invite = await util_1.Invite.findOne({
        ...
        const invite = await util_1.Invite.find({
            where: { guild_id: guild_id, vanity_url: true },
        });
...
router.patch("/", (0, api_1.route)({ body: "VanityUrlSchema", permission: "MANAGE_GUILD" }), async (req, res) => {
    ...
    if (!guild.features.includes("VANITY_URL"))
        throw new lambert_server_1.HTTPError("Your guild doesn't support vanity urls");
    if (!code || code.length === 0)
        throw new lambert_server_1.HTTPError("Code cannot be null or empty");
    const invite = await util_1.Invite.findOne({ where: { code } });
    if (invite)
        throw new lambert_server_1.HTTPError("Invite already exists");
    const { id } = await util_1.Channel.findOneOrFail({
        where: { guild_id, type: util_1.ChannelType.GUILD_TEXT },
    });
    await util_1.Invite.create({
        vanity_url: true,
        code: code,
        temporary: false,
        uses: 0,
