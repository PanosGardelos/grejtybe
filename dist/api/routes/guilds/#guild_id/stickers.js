...
router.post("/", bodyParser, (0, api_1.route)({
...
router.patch("/:sticker_id", (0, api_1.route)({
    body: "ModifyGuildStickerSchema",
    permission: "MANAGE_EMOJIS_AND_STICKERS",
}), async (req, res) => {
    const { guild_id, sticker_id } = req.params;
    const body = req.body;
    const sticker = await util_1.Sticker.create({
        ...body,
        guild_id,
        id: sticker_id,
    }).save();
...
async function sendStickerUpdateEvent(guild_id) {
    return (0, util_1.emitEvent)({
        ...
        data: {
            guild_id: guild_id,
            stickers: await util_1.Sticker.find({ where: { guild_id: guild_id } }),
        },
    });
}
