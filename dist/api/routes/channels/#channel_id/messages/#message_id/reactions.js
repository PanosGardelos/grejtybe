...
router.delete("/:emoji", (0, api_1.route)({ permission: "MANAGE_MESSAGES" }), async (req, res) => {
    ...
    const already_added = message.reactions.find((x) => (x.emoji.id === emoji.id && emoji.id) ||
...
router.get("/:emoji", (0, api_1.route)({ permission: "VIEW_CHANNEL" }), async (req, res) => {
    ...
    const reaction = message.reactions.find((x) => (x.emoji.id === emoji.id && emoji.id) ||
        x.emoji.name === emoji.name);
    if (!reaction)
        throw new lambert_server_1.HTTPError("Reaction not found", 404);
...
router.put("/:emoji/:user_id", (0, api_1.route)({ permission: "READ_MESSAGE_HISTORY", right: "SELF_ADD_REACTIONS" }), async (req, res) => {
    ...
    if (emoji.id) {
        const external_emoji = await util_1.Emoji.findOneOrFail({
            where: { id: emoji.id },
        });
        if (!already_added)
    ...
    const member = channel.guild_id &&
        (await util_1.Member.findOneOrFail({ where: { id: req.user_id } }));
    await (0, util_1.emitEvent)({
