...
router.get("/:code", (0, api_1.route)({}), async (req, res) => {
    ...
    const invite = await util_1.Invite.findOneOrFail({
        where: { code },
...
router.post("/:code", (0, api_1.route)({ right: "USE_MASS_INVITES" }), async (req, res) => {
    const { code } = req.params;
    const { guild_id } = await util_1.Invite.findOneOrFail({
        where: { code: code },
    });
    const { features } = await util_1.Guild.findOneOrFail({
        where: { id: guild_id },
    });
    const { public_flags } = await util_1.User.findOneOrFail({
        where: { id: req.user_id },
    });
    if (features.includes("INTERNAL_EMPLOYEE_ONLY") &&
        (public_flags & 1) !== 1)
        throw new lambert_server_1.HTTPError("Only intended for the staff of this server.", 401);
    if (features.includes("INVITES_CLOSED"))
        throw new lambert_server_1.HTTPError("Sorry, this guild has joins closed.", 403);
