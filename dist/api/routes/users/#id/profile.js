...
router.get("/", (0, api_1.route)({ test: { response: { body: "UserProfileResponse" } } }), async (req, res) => {
    ...
    const user = await util_1.User.getPublicUser(req.params.id, {
        relations: ["connected_accounts"],
    });
    const mutual_guilds = [];
    let premium_guild_since;
    if (with_mutual_guilds == "true") {
        const requested_member = await util_1.Member.find({
            where: { id: req.params.id },
        });
        const self_member = await util_1.Member.find({
    ...
    const guild_member = guild_id && typeof guild_id == "string"
        ? await util_1.Member.findOneOrFail({
